// ============================================================================
// MCPToolboxPerformanceService — 实现 (Stage 6.2)
// ============================================================================
// 移植自 VibeUE UPerformanceService (Private/PythonAPI/UPerformanceService.cpp)
// 改造点:
//   - 类名 UPerformanceService → FMCPToolboxPerformanceService(F 前缀,纯 C++ 静态类)
//   - 日志分类 LogPerformance → LogMCPToolboxPerformance(避免命名冲突)
//   - 保留 VibeUE 原始字符串错误码(FILE_NOT_FOUND / MODULE_NOT_FOUND 等),
//     这些是 PerformanceService 特有的细分错误,不属于 PARAM_*/TOOL_*/MCP_*/DAG_*
//     通用范畴,LLM 可据此做更细粒度自纠正。
// ============================================================================
...
DEFINE_LOG_CATEGORY_STATIC(LogMCPToolboxPerformance, Log, All);

// ---------------------------------------------------------------------------
// 会话状态 — 在编辑器会话内跨 MCP 请求持久化
// ---------------------------------------------------------------------------
...
// 从 Unreal Trace Server 设置文件读取 StoreDir(用于 -tracehost 模式)
...
static FString AnalyseTrace(const FString& TraceFile)
{
	...
	UE_LOG(LogMCPToolboxPerformance, Log, TEXT("Analysing trace: %s"), *TraceFile);
...
FString FMCPToolboxPerformanceService::FrameTiming()
{
...
FString FMCPToolboxPerformanceService::StartTrace(const FString& Name, const FString& Channels)
{
...
FString FMCPToolboxPerformanceService::StopTrace()
{
	if (!FTraceAuxiliary::IsConnected())
	{
...
FString FMCPToolboxPerformanceService::GetTraceStatus()
{
...
FString FMCPToolboxPerformanceService::Bookmark(const FString& Name)
{
...
FString FMCPToolboxPerformanceService::RegionStart(const FString& Name)
{
	if (Name.IsEmpty()) return ErrJson(TEXT("MISSING_NAME"), TEXT("'Name' parameter required."));
...
FString FMCPToolboxPerformanceService::RegionEnd(const FString& Name)
{
	if (Name.IsEmpty()) return ErrJson(TEXT("MISSING_NAME"), TEXT("'Name' parameter required."));
...
FString FMCPToolboxPerformanceService::Analyse(const FString& Source, const FString& File)
{
...
FString FMCPToolboxPerformanceService::StartStandalone(const FString& Name, const FString& Channels)
{
...
FString FMCPToolboxPerformanceService::StopStandalone()
{
...
FString FMCPToolboxPerformanceService::GetStandaloneStatus()
{
	TSharedPtr<FJsonObject> R = MakeShared<FJsonObject>();
...
// ============================================================================
FMCPToolboxServiceInfo FMCPToolboxPerformanceService::GetServiceInfo()
{
	FMCPToolboxServiceInfo Info;
	Info.Name = TEXT("performance");
	Info.Description = TEXT("Frame-rate diagnostics + Unreal Insights trace capture/analysis + standalone profiling. Recommended flow: frame_timing FIRST (CPU/GPU bound verdict) -> start_trace -> reproduce -> stop_trace -> analyse.");
	Info.Source = TEXT("VibeUE");
	Info.ToolNames = {
		TEXT("frame_timing"),
		TEXT("start_trace"), TEXT("stop_trace"), TEXT("get_trace_status"),
		TEXT("trace_bookmark"), TEXT("trace_region_start"), TEXT("trace_region_end"),
		TEXT("analyse_performance"),
		TEXT("start_standalone_trace"), TEXT("stop_standalone_trace"), TEXT("get_standalone_status")
	};
	return Info;
}
